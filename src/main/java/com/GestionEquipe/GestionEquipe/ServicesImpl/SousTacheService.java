...
import com.GestionEquipe.GestionEquipe.Repository.SousTacheRepository;
import com.GestionEquipe.GestionEquipe.Services.ISousTacheService;
...
public class SousTacheService implements ISousTacheService {
    ...
    public SousTache updateSousTask(SousTache sousTache, Integer id) {
        ...
        if (Objects.nonNull(sousTache.getPriorité())
                && !"".equalsIgnoreCase(sousTache.getPriorité())) {
            soustacheDB.setPriorité(
                    sousTache.getPriorité());
        }
        ...
        if (Objects.nonNull(sousTache.getEchéance())
                && !"".equalsIgnoreCase(String.valueOf(sousTache.getEchéance()))) {
            soustacheDB.setEchéance(
                    sousTache.getEchéance());
        }
